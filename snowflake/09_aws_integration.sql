-- ============================================================================
-- 09_AWS_INTEGRATION.SQL — AWS services for Fabric Supply Chain
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION aws_vietnam_textile_supply_chain_BEDROCK_EAI
  ALLOWED_NETWORK_RULES = (TEXTILE_SUPPLY_CHAIN.APP.BEDROCK_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (TEXTILE_SUPPLY_CHAIN.APP.AWS_BEDROCK_SECRET)
  ENABLED = TRUE
  COMMENT = 'Bedrock access for Fabric Supply Chain';
...
CREATE OR REPLACE FUNCTION APP.BEDROCK_GENERATE(prompt VARCHAR)
  ...
  EXTERNAL_ACCESS_INTEGRATIONS = (aws_vietnam_textile_supply_chain_BEDROCK_EAI)
  SECRETS = ('aws_creds' = TEXTILE_SUPPLY_CHAIN.APP.AWS_BEDROCK_SECRET)
AS $$
...
CREATE OR REPLACE STORAGE INTEGRATION aws_vietnam_textile_supply_chain_S3_INT
  TYPE = EXTERNAL_STAGE
  STORAGE_PROVIDER = 'S3'
  STORAGE_AWS_ROLE_ARN = 'arn:aws:iam::018437500440:role/snowflake-sea-demos-s3'
  ENABLED = TRUE
  STORAGE_ALLOWED_LOCATIONS = ('s3://sea-aws-demos-018437500440/aws-vietnam-textile-supply-chain/');

-- External stage for data landing
CREATE OR REPLACE STAGE RAW.LANDING_STAGE
  STORAGE_INTEGRATION = aws_vietnam_textile_supply_chain_S3_INT
  URL = 's3://sea-aws-demos-018437500440/aws-vietnam-textile-supply-chain/';

-- ==================== KINESIS / IOT CORE INGESTION ====================
-- Snowpipe from Kinesis Data Stream
-- Stream ARN: arn:aws:kinesis:ap-southeast-1:018437500440:stream/aws-vietnam-textile-supply-chain-stream

CREATE OR REPLACE PIPE RAW.REALTIME_PIPE
  AUTO_INGEST = TRUE
  INTEGRATION = 'aws_vietnam_textile_supply_chain_S3_INT'
